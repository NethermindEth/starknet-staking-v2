...
docker run nethermind/starknet-staking-v2 \
    --provider-http "http://localhost:6060/rpc/v0_10" \
    --provider-ws "ws://localhost:6061/ws/v0_10" \
...
```bash
PROVIDER_HTTP_URL="http://localhost:6060/v0_10" \
PROVIDER_WS_URL="ws://localhost:6061/v0_10" \
...
```bash
docker run \
  -e PROVIDER_HTTP_URL="http://host.docker.internal:6060/v0_10" \
  -e PROVIDER_WS_URL="ws://host.docker.internal:6061/v0_10" \
